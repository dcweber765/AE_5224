...
std_dev_mag = 5*pi/180;					% Magnetometer std. dev. (rad)
std_dev_gyro= 1*pi/180;					% Rate gyro std. dev. (rad/s)
bias_mag	= 0*pi/180 * randn(3, 1);	% Magnetometer bias (rad)
...
R_me_covariance		= (std_dev_mag^2) * eye(3);								% Measurement error covariance (3x3 matrix)
P_ee_covariance     = blkdiag(R_me_covariance, eye(6));						% I_6 is for biases
...
for m1 = 2:n_pts
	...
	A_e	= [eye(3) zeros(3) -Hinv*dt; zeros(3) eye(3) zeros(3); zeros(3,6) eye(3)];
	B_e	= [Hinv*dt zeros(3, 6); zeros(6, 3) eye(6)*dt];
	Q_pn_covariance = B_e * (blkdiag( (std_dev_gyro^2) * eye(3), 0*eye(6))) * B_e';
	...
	% Measurement model
    C_e	= [eye(3) eye(3) zeros(3)];		% For Euler angles
	
	% Kalman gain
    K_e_gain = P_minus*C_e' / (C_e*P_minus*C_e' + R_me_covariance);			%... as does the Kalman gain computation
	
	% Recursive WLSE method to update predictive estimate with new
	% measurement
	x_e_hat	= x_e_minus + K_e_gain * ( zm - C_e*x_e_minus );				% New estimate
	P_ee_covariance		= (eye(9) - K_e_gain * C_e) * P_minus;				% New e.e. covariance, again linearized C 
	
	% Record
	euler_angles_ekf_est(:, m1)			= x_e_hat(1:3);
	biases_e_ekf_est(:, m1)				= x_e_hat(4:end);
